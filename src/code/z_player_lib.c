...
s32 Player_OverrideLimbDrawGameplayDefault(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
    ...
    if (!Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            if (LINK_IS_CHILD && this->leftHandType == PLAYER_MODELTYPE_LH_HAMMER && this->heldItemActionParam == PLAYER_AP_HAMMER) {
                *dList = gLinkChildLeftHandHoldingHammerNearDL;
            } else {
            ...
            if (LINK_IS_CHILD && this->rightHandType == PLAYER_MODELTYPE_RH_HOOKSHOT) {
...
void Player_DrawHookshotReticle(PlayState* play, Player* this, f32 arg2) {
    ...
    if (BgCheck_AnyLineTest3(&play->colCtx, &sp8C, &sp80, &sp74, &sp9C, 1, 1, 1, 1, &bgId)) {
        ...
        objBankIndexGameplayKeep = Object_GetIndex(&play->objectCtx, OBJECT_GAMEPLAY_KEEP);
        if (objBankIndexGameplayKeep >= 0) {
            gSPSegment(OVERLAY_DISP++, 0x06, play->objectCtx.status[objBankIndexGameplayKeep].segment);
